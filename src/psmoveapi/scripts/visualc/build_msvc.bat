...
REM Build OpenCV
echo.
@REM echo Building OpenCV ^(Debug^)
@REM msbuild.exe %OPENCV_DIR%/build-%MSVC_PLATFORM%/ALL_BUILD.vcxproj /p:Configuration=Debug /property:Platform=%MSVC_PLATFORM% /verbosity:minimal
@REM IF %ERRORLEVEL% NEQ 0 (
@REM 	echo Failed to build OpenCV ^(Debug^)
@REM 	goto Error
@REM )
@REM echo.
...
cmake .. -G "%MSVC_CMAKE_GENERATOR%" -A "%MSVC_CMAKE_ARCH%" -DPSMOVE_USE_PS3EYE_DRIVER=ON -DPSMOVE_BUILD_TRACKER=ON -DOpenCV_DIR=./external/opencv/build-%MSVC_PLATFORM%/
IF !ERRORLEVEL! NEQ 0 (
...
REM Build PSMoveAPI
echo.
@REM echo Building PSMoveAPI ^(Debug^)
@REM msbuild.exe %PSMOVE_API_ROOT_DIR%/build-%MSVC_PLATFORM%/ALL_BUILD.vcxproj /p:Configuration=Debug /property:Platform=%MSVC_PLATFORM% /verbosity:minimal
@REM IF %ERRORLEVEL% NEQ 0 (
@REM 	echo Failed to build PSMoveAPI ^(Debug^)
@REM 	goto Error
@REM )
@REM echo.
